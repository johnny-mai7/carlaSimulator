    ...
    if args.map is not None:
        ...
        if os.path.exists(args.xodr_path):
            with open(args.xodr_path, encoding='utf-8') as od_file:
                try:
                    data = od_file.read()
                    print(f"Loaded XODR data from {args.xodr_path}: {len(data)} bytes")
                except OSError:
        ...
        if os.path.exists(args.osm_path):
            with open(args.osm_path, encoding='utf-8') as osm_file:
                try:
                    data = osm_file.read()
                    print(f"Loaded OSM data from {args.osm_path}: {len(data)} bytes")
                except OSError:
                    print('file could not be read.')
                    sys.exit()
            print('Converting OSM data to OpenDRIVE')
            xodr_data = carla.Osm2Odr.convert(data)
            print('Converted OSM data to OpenDRIVE')
